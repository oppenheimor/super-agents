...
export function truncateToolResults(
  ...
  const result = messages.map((msg) => {
    ...
    const newContent = msg.content.map((part: any) => {
      ...
      if (part.output.length <= config.maxSingleResult) {
        return part;
      }
...
export function applyDefense(messages: ModelMessage[], timestamps: Map<number, number>) {
  // Layer2: truncate oversized tool results
  const trunc = truncateToolResults(messages);
  let result = trunc.messages;

  // Layer3: TTL prune old tool results
  const prune = ttlPrune(result, timestamps);
  result = prune.messages;

  // Layer1: estimate final token count
