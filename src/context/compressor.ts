import { generateText, ModelMessage, LanguageModel } from 'ai';
...
export function microcompact(messages: ModelMessage[]): {
  ...
  for (let i = 0; i < messages.length; i++) {
    if (messages[i].role === 'tool' && Array.isArray(messages[i].content)) {
...
export async function summarize(
  ...
  const toCompress: ModelMessage[] = messages.slice(0, alignedIdx);
